<template>
    <div class="file__input__wrapper">
        ...
        >
            ...
            <p> {{ !file || file.name.length <= 0 ? (placeholder || "파일을 선택하거나 이곳으로 드래그 해주세요.") : file.name }} </p>
            <input :id="uuid" type='file' title="min is " placeholder="asdfasdf" @change="(e) => fileUploadHandler(e)"/>
        </label>
        <i v-if="fileName" @click.stop.prevent="(e) => fileUploadHandler(e)"></i>
    </div>
</template>
